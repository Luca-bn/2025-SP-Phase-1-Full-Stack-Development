...
export default function Checkout({ ref }: { ref: Ref<unknown> }) {
  const checkoutDialogRef = useRef<HTMLDialogElement>(null);
  const cartContext = useContext(CartContext);
  const { sent, data, error, httpCall, resetHttpState } = useHttp<string>();
    ...
    async function call() {
      const body = JSON.stringify({
        order: {
          ...
          customer: Object.fromEntries(new FormData(event.target as HTMLFormElement).entries()),
        },
      });
